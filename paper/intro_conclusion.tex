...
\textbf{(ii)}~a two-stage CRPS$\rightarrow$NLL training scheme that resolves the NLL cold-start, with an ablation against direct-NLL training quantifying the gain;
...
\textbf{(iv)}~a calibration analysis (ECE, reliability and PIT diagrams) and a discussion of how the resulting uncertainty feeds risk-aware engineering decisions.
...
To train it reliably, we introduced a two-stage CRPS-then-NLL scheme that eliminates the cold-start instability of training the Gaussian likelihood from scratch; our ablation shows direct-NLL training is both less accurate and less calibrated than the two-stage procedure.

On the WeatherBench t2m benchmark, ProbWrapper attaches cleanly to SimVP, TAU, PredRNN and ConvLSTM, confirming that the method is not tied to any single architecture.
...
Limitations of the present study include a single variable (2-metre temperature), a single spatial resolution ($5.625^\circ$) and a 12-step horizon; extending ProbWrapper to multiple variables, higher resolutions, and longer lead times---together with non-Gaussian predictive heads for multimodal weather regimes---are promising directions for future work.
